...
services:
  mongo:
    image: "mongo:3"
    volumes:
      {{- if eq .Values.volume_type "named" }}- graylog-mongo-volume{{- else}}- ${volume_mnt_src}/mongo/data/db{{- end}}:/data/db
  ...
  elasticsearch:
    ...
    volumes:
      - {{- if eq .Values.volume_type "named" }}graylog-elasticsearch-volume{{- else}}${volume_mnt_src}/elasticsearch/data{{- end}}:/usr/share/elasticsearch/data
    labels:
  ...
  graylog:
    ...
    labels:
      ...
      io.rancher.scheduler.affinity:container_label_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
    volumes:
      - {{- if eq .Values.volume_type "named" }}graylog-journal-volume{{- else}}${volume_mnt_src}/graylog/data/journal{{- end}}:/usr/share/graylog/data/journal
    volumes_from:
      - geoip-data
    links:
      - elasticsearch:elasticsearch
  ...
  geoip-data:
    ...
    tty: true
    volumes:
      - {{- if eq .Values.volume_type "named" }}graylog-geodata-volume{{- else}}${volume_mnt_src}/geoip{{- end}}:/usr/share/graylog/data/geoip
    environment:
